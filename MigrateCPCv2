    ...
    try {
        ...
        foreach ($policy in $policies.value) {
            ...
            $policyInfo = [PSCustomObject]@{
                ...
                DomainJoinType = $policy.domainJoinType
                Type = $policy.type
...
do {
    ...
    switch ($choice) {
        ...
        "2" {
            ...
            # Step 2: Get and display policies
            $policies = Get-CloudPCProvisioningPolicies
            if (-not $policies) {
            ...
            # Step 3: Select source policy (optional)
            ...
            # Step 4: Select target/recovery policy (required)
            ...
            # Step 5: Move user between policy groups
            Write-Host "`nMoving user between provisioning policies..." -ForegroundColor Yellow
            $moveResult = Move-UserBetweenPolicyGroups -UserPrincipalName $userPrincipalName -SourcePolicyName $sourcePolicyName -SourceGroupId $sourceGroupId -TargetPolicyName $targetPolicyName -TargetGroupId $targetGroupId
            
            if (-not $moveResult) {
                Write-Host "Failed to move user. Recovery workflow stopped." -ForegroundColor Red
                Read-Host "`nPress Enter to continue"
                continue
            }
            ...
            if ($cloudPCs) {
                ...
                if ($gracePeriodPCs) {
                    ...
                    $endGrace = Read-Host "Do you want to end the grace period to start reprovisioning with '$targetPolicyName'? (y/n)"
                    ...
                    Write-Host "No Cloud PCs in grace period. New provisioning may begin automatically." -ForegroundColor Green
                }
            }
            ...
            if ($monitor -eq 'y' -or $monitor -eq 'Y') {
                ...
                Write-Host "`nMonitoring for new Cloud PC provisioning using '$targetPolicyName' policy..." -ForegroundColor Green
            ...
            Write-Host "User has been moved to '$targetPolicyName' provisioning policy" -ForegroundColor Green
            Read-Host "`nPress Enter to continue"
        }
        ...
        default {
...
} while ($choice -ne "7"):" -ForegroundColor Yellow
                    for ($i = 0; $i -lt $sourcePolicy.AssignedGroups.Count; $i++) {
                        Write-Host "  [$($i + 1)] $($sourcePolicy.AssignedGroups[$i].GroupName)"
                    }
                    $groupChoice = Read-Host "Enter group number"
            ...
            # Step 4: Select target policy (required)
            Write-Host "`nSelect target (recovery) provisioning policy:" -ForegroundColor Green
            $targetChoice = Read-Host "Enter policy number (1-$($policies.Count))"
        ...
        "6" {
            ...
            if ($sourceChoice -and [int]$sourceChoice -ge 1 -and [int]$sourceChoice -le $policies.Count) {
                ...
                if ($sourcePolicy.AssignedGroups.Count -eq 1) {
                    ...
                    Write-Host "Multiple groups found for source policy. Select group
